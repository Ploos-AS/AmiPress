...
static int object_begin(struct amipdf *pdf, int number)
{
    ...
    return fprintf(pdf->out, "%d 0 obj\n", number) < 0 ? AMIPDF_ERR_IO : AMIPDF_OK;
}
...
static unsigned char *flate_store(const unsigned char *data, size_t len, size_t *out_len)
{
    ...
    ad = adler32(data, len); out[pos++] = (unsigned char)(ad >> 24); out[pos++] = (unsigned char)(ad >> 16);
    out[pos++] = (unsigned char)(ad >> 8); out[pos++] = (unsigned char)ad; *out_len = pos; return out;
}
...
{
    struct amipdf_image *image; size_t bytes; char buf[192]; int n;
    ...
    bytes = (size_t)width * (size_t)height; if (bytes > ((size_t)-1) / 3U) return AMIPDF_ERR_ARGUMENT; bytes *= 3U;
    if (reserve_images(pdf, pdf->image_count + 1) != AMIPDF_OK) return AMIPDF_ERR_MEMORY;
    image = &pdf->images[pdf->image_count]; image->data = (unsigned char *)malloc(bytes); if (!image->data) return AMIPDF_ERR_MEMORY;
...
static int write_page(struct amipdf *pdf, size_t index)
{
    ...
    if (pdf->image_count) {
        ...
        for (i = 0; i < pdf->image_count; ++i)
            if (fprintf(pdf->out, " /Im%lu %d 0 R", (unsigned long)(i + 1), pdf->images[i].object) < 0) return AMIPDF_ERR_IO;
        if (fputs(" >>", pdf->out) == EOF) return AMIPDF_ERR_IO;
    }
    return fprintf(pdf->out, " >> /Contents %d 0 R >>\nendobj\n", page->contents_object) < 0 ? AMIPDF_ERR_IO : AMIPDF_OK;
}
static int write_image(struct amipdf *pdf, struct amipdf_image *image)
{
    unsigned char *compressed; size_t clen;
    compressed = flate_store(image->data, image->data_len, &clen); if (!compressed) return AMIPDF_ERR_MEMORY;
